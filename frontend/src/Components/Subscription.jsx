...
class DefaultSubscription extends Component {
  ...
  connect() {
    this.subscription = this.props.socket.message$.pipe(filter(message => message.channel === this.props.channel)).subscribe(message => {
      switch (message.id) {
  ...
  render() {
    ...
    return (
      <div className={styles['camera-subscription']}>
        ...
        <DefaultSubscription {...this.props} muted={this.state.muted} volume={this.state.volume} onConnected={onConnected} />
      </div>
    )
  }
}
